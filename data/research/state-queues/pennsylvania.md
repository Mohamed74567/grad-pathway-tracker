# Pennsylvania State Queue

## Workflow status

Pennsylvania is the first state processed under the legacy-first workflow. Existing directory records were checked against the uploaded tracker before non-legacy discovery. The current state has **11 published profiles** after the legacy-listed Lehigh doctoral addition, with legacy-listed Drexel, Penn State, Pittsburgh, Carnegie Mellon, and University of Pennsylvania programs already represented.

The legacy tracker supplied relevant Pennsylvania Ph.D. seeds for Carnegie Mellon, Drexel, Penn State, University of Pennsylvania, University of Pittsburgh, and Lehigh. Drexel, Penn State, Pittsburgh, and Lehigh have already received current-cycle source refresh work in this state pass. The original legacy deadlines and notes remain archival evidence only and are not copied into current admission facts without current official confirmation.

| Institution / program | Current directory status | Priority | Official discovery source | Evidence status |
| --- | --- | --- | --- | --- |
| Carnegie Mellon University — BME Ph.D. / research M.S. | Published; legacy-listed | Refresh remaining snapshot gaps | https://www.cmu.edu/bme/ | Current-source refresh pending |
| Drexel University — BME Ph.D. / M.S. | Published; legacy-listed | Completed refresh | https://drexel.edu/biomed/admissions/graduate/ | Direct admission facts refreshed |
| Lehigh University — Bioengineering Ph.D. | Published; legacy-listed | Completed publication | https://engineering.lehigh.edu/bioe/graduate/bioe-admissions-guidelines | Three-pass verified |
| Pennsylvania State University — BME Ph.D. / thesis M.S. | Published; legacy-listed | Completed refresh | https://www.bme.psu.edu/students/graduate/how-to-apply.aspx | Current source-safe snapshot refresh |
| University of Pennsylvania — Bioengineering Ph.D. / M.S.E. | Published; legacy-listed Ph.D. | Refresh remaining snapshot gaps | https://be.engineering.upenn.edu/doctoral/ | Current-source refresh pending |
| University of Pittsburgh — Bioengineering Ph.D. / research M.S. | Published; legacy-listed | Completed refresh | https://www.engineering.pitt.edu/departments/bioengineering/programs/graduate/admissions/graduate-admissions/ | Current-source refresh and hero added |
| Temple University — Bioengineering Ph.D. / MSBioe | Not yet published | Publish after conflict-aware final check | https://engineering.temple.edu/admissions/graduate-admissions | Admissions reconciliation completed; publication queued |
| University of Pittsburgh — professional M.S. Bioengineering, Neural Engineering focus | Published | Completed publication | https://www.engineering.pitt.edu/departments/bioengineering/research/neuro/neuro/ | Three-pass verified; qualified fields only |
| Villanova University — M.S. Biomedical Engineering | Published | Completed publication | https://www.villanova.edu/university/engineering/academic-programs/biomedical/ms-biomedical-engineering.html | Three-pass verified; funding deliberately not stated |

## Evidence rules for this queue

Only department, college, graduate-school, university-catalog, or official university news/photography pages qualify for publication. A candidate moves from **research** to **publish** only after program identity, degree structure, and admissions facts are reconciled across three official passes. Application dates without a stated active cycle are saved as annual guidance, not converted into dated planner items. Fee, GRE, English/Duolingo, acceptance, funding, and imagery fields remain blank unless a directly linked official source states them.
