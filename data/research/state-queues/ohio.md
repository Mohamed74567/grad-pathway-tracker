# Ohio State Queue

## Scope and operating rule

Ohio is the second state in the systematic directory workflow. The queue begins with programs already named in the uploaded legacy tracker, then adds eligible graduate Biomedical Engineering, Bioengineering, Neuroengineering, and closely adjacent programs discovered through official university sources. Every profile must have five source-field categories, three verification passes, a working official program/application route, and credited real campus imagery before publication or final refresh.

## Current directory baseline

| Institution | Published programs | Legacy status | Current provenance status | Next action |
|---|---:|---|---|---|
| Case Western Reserve University | Ph.D. and M.S. Biomedical Engineering | Legacy-listed Ph.D. | Three-pass verified | Completed current admissions/funding refresh; retain annual guidance only |
| The Ohio State University | Ph.D. and M.S. Biomedical Engineering | Not identified in legacy audit | Three-pass verified | Retain; audit link health with the wider directory pass |
| University of Cincinnati | Ph.D. and M.S. Biomedical Engineering | Not identified in legacy audit | Three-pass verified | Retain; audit link health with the wider directory pass |
| Ohio University | M.S. Biomedical Engineering | Not identified in legacy audit | Three-pass verified | Published from official state-queue research |

## Ohio research queue

| Priority | Candidate | Legacy status | Research objective | Official discovery seed |
|---:|---|---|---|---|
| 1 | Case Western Reserve University Biomedical Engineering Ph.D./M.S. | Ph.D. in uploaded tracker | Completed refresh | https://case.edu/bme/academics/graduate/how-apply |
| 2 | University of Dayton Bioengineering graduate program | Legacy-listed as Bioengineering Ph.D.; legacy note suggests master’s context | **Hold — not eligible as a distinct current Bioengineering graduate degree.** Official catalog identifies an M.S. in Chemical Engineering that includes bioengineering technical development, while Bioengineering is an undergraduate co-major/course area. | https://catalog.udayton.edu/graduate/schoolofengineering/programsofstudy/chemicalengineering/ |
| 3 | Ohio University M.S. Biomedical Engineering | Discovery candidate | Completed publication | https://www.ohio.edu/engineering/chemical/graduate/biomedical-engineering |
| 4 | Cleveland State University M.S. Biomedical Engineering | Discovery candidate | Completed publication | https://engineering.csuohio.edu/chemical/master-science-in-biomedical-engineering |
| 5 | Cleveland State University Ph.D. in Engineering — Applied Biomedical Engineering specialization | Discovery candidate | Hold for further current admissions-policy research; degree and assistantship structure confirmed | https://engineering.csuohio.edu/chemical/applied-biomedical-engineering-doctoral-program |
| 6 | Miami University M.Eng. Biomedical Engineering | Discovery candidate | Research and reconcile | https://programs.miamioh.edu/programs/biomedical-engineering-m-eng/ |
| 7 | Wright State University M.S. Biomedical Engineering | Discovery candidate | Completed publication | https://engineering-computer-science.wright.edu/biomedical-industrial-and-human-factors-engineering/master-of-science-in-biomedical-engineering |
| 8 | Miami University M.Eng. Biomedical Engineering | Discovery candidate | Completed publication under Miami University of Ohio sources only | https://programs.miamioh.edu/programs/biomedical-engineering-m-eng/ |

## Guardrails

No legacy deadline, fee-waiver note, or policy is carried forward as fact without current official confirmation. Historical links are useful research seeds only. A program without a currently confirmed qualifying graduate degree remains in this queue rather than being published.

## University of Dayton verification outcome

The uploaded legacy tracker’s “PhD in Bioengineering” seed does **not** correspond to a currently confirmed Bioengineering doctoral program. The official School of Engineering catalog identifies only a Master of Science in Chemical Engineering; its curriculum may include bioengineering coursework and applications, but it is not a graduate Bioengineering degree. The official Bioengineering designation is a co-major at the undergraduate level. Therefore no Dayton profile is published under the tracker’s qualifying graduate-program taxonomy.

## Cohort validation status

The current Ohio cohort contains **10 published profiles**, each with credited real campus imagery and all five source-field categories at three verification passes. The state-wide discovery pass confirmed the currently published group and identified no additional distinct qualifying University of Akron or Kent State biomedical-engineering graduate degree for publication. Cleveland State’s Ph.D. in Engineering — Applied Biomedical Engineering specialization remains intentionally held: the degree, research structure, and conditional Cleveland Clinic research-assistantship pathway are verified, but a current official doctoral admissions-requirements page has not yet provided a safe GRE/fee/English/deadline snapshot. It will be revisited rather than published with inferred facts.

## The Ohio State University Biomedical Engineering fee-waiver refresh — August 27, 2026

The current Ohio State Biomedical Engineering admissions page states that the program does not issue fee waivers directly. It identifies possible resources for domestic applicants who attended the October Ohio State Graduate Engineering Open House or participated in SROP, McNair Scholars, or Buckeye REU programs, and points applicants to the Big Ten Academic Alliance FreeApp program. In rare cases a faculty member may be able to assist, but the department publishes no guaranteed code or automatic waiver. The current BME application handbook lists a nonrefundable fee of US$60 for domestic applicants and US$70 for international applicants and discusses traditional M.S. and Ph.D. applicants together. The guidance is therefore shown on both published degree paths as a shared opportunity notice, without transferring an unsupported degree-specific policy. The separate central Graduate Admissions application route remains linked for confirmation of current mechanics.

Sources reviewed: [Ohio State BME before applying](https://bme.osu.edu/graduate-school-biomedical-engineering-ohio-state/applying-biomedical-engineering-graduate-programs), [Ohio State BME application steps](https://bme.osu.edu/graduate-school-biomedical-engineering-ohio-state/applying-biomedical-engineering-graduate-programs/apply-bme-graduate-programs-step-step), and [Ohio State BME M.S./Ph.D. application handbook](https://bme.osu.edu/section-16-ms-and-phd-application-admission-requirements).

Validation: focused directory suite passed all 191 tests after strengthening assertions for the current US$60/$70 fee schedule, domestic opportunity categories, no BME-issued code, and shared M.S./Ph.D. scope.
