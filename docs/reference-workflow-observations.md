...
## Program Explorer loading-state validation

After a development-service restart, the first `/programs` visual capture showed the app-shell skeleton while the route and data work initialized. The repaired Directory view then displayed its dedicated three-row loading surface rather than the earlier contradictory combination of “Loading verified records” and an empty-result message. A later populated `/programs` capture showed the PhD tab with **64 source-checked programs** and visible compact program rows, including Boston University. The short-lived first-load state is now represented as an honest loading surface; the empty-result explanation is reserved for a completed query with zero matches.
