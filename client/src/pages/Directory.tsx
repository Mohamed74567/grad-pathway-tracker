...
export default function Directory() {
  ...
  return <div className="content-frame page-enter space-y-5">
    ...
    <section className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">{degreeTitle}</p><h2 className="mt-1 text-xl font-extrabold tracking-tight">{isLoading ? "Loading verified records" : isError ? "Directory temporarily unavailable" : `${filteredPrograms.length} source-checked programs`}</h2></div><Button variant="outline" onClick={clearFilters} className="rounded-xl border-slate-200 bg-white"><FilterX className="mr-2 h-4 w-4" />Reset explorer</Button></section>
    {isLoading ? <div className="surface space-y-3 p-5" aria-busy="true" aria-label="Loading verified program records"><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /></div> : isError ? <div className="surface py-16 text-center"><SlidersHorizontal className="mx-auto h-8 w-8 text-violet-400" /><h3 className="mt-4 text-lg font-bold">The directory could not load</h3><p className="mx-auto mt-1 max-w-md text-sm text-slate-600">Please refresh this view. Your saved applications are unchanged.</p></div> : filteredPrograms.length ? <div className="space-y-3">{filteredPrograms.map(program => <ProgramListRow key={program.id} program={program as DirectoryListProgram} saved={saved.has(program.id)} onSave={id => add.mutate({ programId: id })} />)}</div> : <div className="surface py-16 text-center"><SlidersHorizontal className="mx-auto h-8 w-8 text-violet-400" /><h3 className="mt-4 text-lg font-bold">No verified records match this view</h3><p className="mx-auto mt-1 max-w-md text-sm text-slate-600">Try a broader filter. Programs with unknown GRE or English-test policies are deliberately not included in those policy filters.</p></div>}
  </div>;
}
