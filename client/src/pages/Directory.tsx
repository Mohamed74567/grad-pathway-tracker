import { type DirectoryListProgram } from "@/components/ProgramListRow";
import { UniversityProfileRow, type UniversityProfile } from "@/components/UniversityProfileRow";
...
import { groupUniversityProfiles } from "../../../shared/universityProfiles";
...
export default function Directory() {
  ...
  const input = useMemo(() => ({ subfields: subfield === "all" ? undefined : [subfield], states: state === "all" ? undefined : [state], funding: funding === "all" ? undefined : [funding as "funded" | "available" | "not_stated" | "not_applicable"], tiers: tier === "all" ? undefined : [tier as "q1" | "q2" | "q3" | "not_listed"], search: deferredSearch.trim() || undefined }), [deferredSearch, funding, state, subfield, tier]);
  const { data: programs = [], isLoading, isError } = trpc.tracker.directory.list.useQuery(input);
  ...
  const supplementalPrograms = programs.filter(program => matchesDirectorySupplementalFilters(program, {
  ...
  const filteredPrograms = supplementalPrograms.filter(program => degree === "all" || program.degreeType === degree);
  const universityProfiles = useMemo(() => {
    const visibleIds = new Set(filteredPrograms.map(program => program.id));
    return groupUniversityProfiles(programs as DirectoryListProgram[], visibleIds);
  ...
  return <div className="content-frame page-enter space-y-5">
    ...
    <section className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">{degreeTitle}</p><h2 className="mt-1 text-xl font-extrabold tracking-tight">{isLoading ? "Loading university profiles" : isError ? "Directory temporarily unavailable" : `${universityProfiles.length} university profiles`}</h2><p className="mt-1 text-xs text-slate-500">Each profile keeps every degree path and its application-fee waiver information separate inside the university card.</p></div><Button variant="outline" onClick={clearFilters} className="rounded-xl border-slate-200 bg-white"><FilterX className="mr-2 h-4 w-4" />Reset explorer</Button></section>
    {isLoading ? <div className="surface space-y-3 p-5" aria-busy="true" aria-label="Loading verified university profiles"><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /><div className="h-16 animate-pulse rounded-2xl bg-slate-100" /></div> : isError ? <div className="surface py-16 text-center"><SlidersHorizontal className="mx-auto h-8 w-8 text-violet-400" /><h3 className="mt-4 text-lg font-bold">The directory could not load</h3><p className="mx-auto mt-1 max-w-md text-sm text-slate-600">Please refresh this view. Your saved applications are unchanged.</p></div> : universityProfiles.length ? <div className="space-y-3">{universityProfiles.map(profile => <UniversityProfileRow key={profile.universityName} profile={profile as UniversityProfile} />)}</div> : <div className="surface py-16 text-center"><SlidersHorizontal className="mx-auto h-8 w-8 text-violet-400" /><h3 className="mt-4 text-lg font-bold">No verified records match this view</h3><p className="mx-auto mt-1 max-w-md text-sm text-slate-600">Try a broader filter. Programs with unknown GRE or English-test policies are deliberately not included in those policy filters.</p></div>}
  </div>;
}
