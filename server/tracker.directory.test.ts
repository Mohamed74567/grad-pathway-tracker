...
describe("tracker.directory", () => {
  ...
  it("returns University of Iowa Biomedical Engineering degree paths with the qualified Graduate Admissions fee-waiver route", async () => {
    ...
    for (const program of paths) {
      expect(program?.applicationFeeDisplay).toBe("US$60 domestic; US$100 international");
      expect(program?.applicationGuidance).toEqual(expect.arrayContaining([
        expect.objectContaining({
          guidanceType: "fee_waiver_form",
  ...
  it("returns University of Maryland Bioengineering degree paths with the qualified Graduate School fee-waiver route", async () => {
    ...
    for (const program of paths) {
      expect(program?.applicationFeeDisplay).toBe("US$90 non-refundable application fee for each program");
      expect(program?.applicationGuidance).toEqual(expect.arrayContaining([
        expect.objectContaining({
          guidanceType: "fee_waiver_form",
  ...
  it("returns UVA Biomedical Engineering degree paths with the current Engineering-wide no-fee policy", async () => {
    ...
    for (const program of paths) {
      expect(program?.applicationFeeDisplay).toBe("No application fee for UVA Engineering graduate programs");
      expect(program?.applicationGuidance).toEqual(expect.arrayContaining([
        expect.objectContaining({
          guidanceType: "fee_waiver_form",
  ...
  it("preserves Mississippi State Biomedical Engineering fee treatment and central-policy boundaries on both degree paths", async () => {
    const caller = appRouter.createCaller(createUnauthenticatedContext());
    const programs = await Promise.all([
      caller.tracker.directory.bySlug({ slug: "mississippi-state-biomedical-engineering-phd" }),
      caller.tracker.directory.bySlug({ slug: "mississippi-state-biomedical-engineering-ms" }),
    ]);
    ...
    for (const program of programs) {
      ...
      expect(program?.applicationGuidance).toEqual(expect.arrayContaining([
        expect.objectContaining({
          guidanceType: "fee_waiver_form",
          details: expect.stringContaining("do not publish a general application-fee waiver"),
          verificationPasses: 3,
        }),
      ]));
    }
  ...
  it("preserves Drexel Biomedical Engineering fee treatment when School of Engineering waiver pages conflict", async () => {
    ...
    const masters = await caller.tracker.directory.bySlug({ slug: "drexel-biomedical-engineering-ms" });
  ...
  it("preserves CCNY Biomedical Engineering fee treatment when central and department waiver statements conflict", async () => {
    ...
    const masters = await caller.tracker.directory.bySlug({ slug: "ccny-biomedical-engineering-ms" });
